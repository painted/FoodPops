class ReviewsController < ApplicationController
	...
	def create
		...
		if foody_review_today.present?
			flash[:notice] = 'Unfortunately, you have met your daily limit of reviews for this event.'
		elsif @event.reviews.where("foody_id = ? AND created_at > ?", @user_id, 31.days.ago).count == 5
	...
	def foody_review_today
		@event.reviews.where("foody_id = ? AND created_at > ?", @user_id, 1.day.ago)
	end
